...
const pillars = [
  ...
  { title: "NR-10 / NR-35", sub: "Segurança elétrica" },
  { title: "TOTVS ERP", sub: "Rastreabilidade total" },
        ...
        <div className="grid gap-16 lg:grid-cols-2">
          ...
          <div>
            ...
            <h2 className="mb-6 font-display text-[clamp(2rem,3vw,2.75rem)] font-bold leading-tight tracking-tight text-white">
              {company.founded}. Uma empresa integradora nasce em Sertãozinho, SP.
            </h2>
            <p className="mb-8 text-base leading-relaxed text-zinc-400">
              Fundada para integrar sistemas elétricos e de automação em plantas industriais
              críticas, a Authomathika tornou-se referência em EPC, energias renováveis e
              manutenção especializada. Mais de duas décadas de execução com segurança e qualidade.
            </p>
          ...
          {/* ── Right: client grid ── */}
          <div>
            <p className="mono-label-muted mb-6">Clientes de referência</p>
            <div className="grid grid-cols-3 gap-2">
              {clients.map((client) => (
                <div
                  key={client}
                  className="flex items-center justify-center rounded-xl border border-zinc-800/80 bg-zinc-900/40 px-3 py-4 text-center text-xs font-medium text-zinc-400 transition-colors hover:border-zinc-700 hover:text-zinc-200"
                >
                  {client}
                </div>
              ))}
